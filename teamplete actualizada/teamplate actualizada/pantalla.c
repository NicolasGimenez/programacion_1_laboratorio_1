...
int lista_pantalla_ordenada(Pantalla* arrayPantalla,int limite, int orden)
{
    int retorno = -1;
    int i;
    int flagSwap;
    ...
    if(limite > 0 && arrayPantalla != NULL)
    {
        do
        {
            ...
            for(i=0;i<limite-1;i++)
            {
                ...
                {
                    if((arrayPantalla[i].precio > arrayPantalla[i+1].precio  && orden)|| (arrayPantalla[i].precio < arrayPantalla[i+1].precio && !orden)) //******
                    {
                    ...
                    {
                        if(arrayPantalla[i].precio==arrayPantalla[i+1].precio)
                        {
...
int pantalla_mostrar_precio(Pantalla* array,int limite)
{
    int retorno = -1;
    int i;
    if(limite > 0 && array != NULL)
    {
        ...
        {
            if(!array[i].isEmpty)
                printf("[RELEASE] - %d - %s - %d - %f\n",array[i].idPantalla, array[i].nombre, array[i].isEmpty,array[i].precio);
        }
    }
    return retorno;
}
int pantalla_mostrar_precio_mayorIgual(Pantalla* array,int limite)
{
    int retorno = -1;
    int i;
    if(limite > 0 && array != NULL)
    {
        retorno = 0;
        printf("\nlas pantallas inferiores a 1000 pesos\n");
        for(i=0;i<limite;i++)
        {
            if(!array[i].isEmpty)
            {
                if(array[i].precio < 1001)
                {
                    printf("[RELEASE] - %d - %s - %d - %f\n",array[i].idPantalla, array[i].nombre, array[i].isEmpty,array[i].precio);
                }
            }
...
int promedio_precio_pantalla(Pantalla* array,int limite)
{
    float sumadorPrecio = 0;
           ...
           if(!array[i].isEmpty)
            {
                ...
                sumadorPrecio = sumadorPrecio + array[i].precio;
            }
        }
        promedioPrecio = sumadorPrecio / contadorPrecio;
        for(i=0;i<limite;i++)
            {
               if(!array[i].isEmpty && array[i].precio > promedioPrecio )
               {
                    printf("[RELEASE] - %d - %s - %d - %f\n",array[i].idPantalla, array[i].nombre, array[i].isEmpty,array[i].precio);
               }
            }
